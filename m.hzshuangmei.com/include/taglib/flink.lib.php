            ...
            if(!empty($linkInfo)){
                ...
                for($i=0; $i<count($dedelink); $i++) {
                    if($i%5==0 && $i!=count($dedelink)) {
                        $revalue .= "<a href='http://".@$dedelink[$i+1]."' target='_blank' title='".@$dedelink[$i+4]."'>".@$dedelink[$i]."</a>";
                    }
                }
            } else {
                $revalue=<<<EOT
...
EOT;
    ...
    while($dbrow=$dsql->GetObject())
  {
      ...
      {
       $link = "<a href='".$dbrow->url."' target='_blank'>".cn_substr($dbrow->webname,$titlelen)."</a> ";
         if($dbrow->nofollow==2){$link = "<a href='".$dbrow->url."' rel='nofollow' target='_blank'>".cn_substr($dbrow->webname,$titlelen)."</a> ";}
     }
      else if($type=='image')
        {
          $link = "<a href='".$dbrow->url."' target='_blank'><img src='".$dbrow->logo."' width='88' height='31' border='0'></a> ";
      if($dbrow->nofollow==2)$link = "<a href='".$dbrow->url."' rel='nofollow' target='_blank'><img src='".$dbrow->logo."' width='88' height='31' border='0'></a> ";
       }
     else
       {
          if($dbrow->logo=='')

...
$dsql->SetQuery( "SELECT * FROM #@__archives AS a,#@__addondoctors AS b
where a.id =b.aid and a.typeid='$relatedoctortypeid' order by rand() limit 2 ");
$dsql->Execute();
$ns = $dsql->GetTotalRow();
while($row=$dsql->GetArray())
{
...
$url = $urlarray['arcurl'];
$thumb = replaceurl($row["thumb"]);
$commenddoctor.= '<a href="'.$url .'" target="_blank"><img src="'.$thumb .'" alt="'.$title.'"/></a>';

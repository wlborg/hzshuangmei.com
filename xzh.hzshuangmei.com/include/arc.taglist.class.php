...
class TagList
{
    ...
    function CountRecord()
    {
        ...
        if($this->TotalResult==-1)
        {
           ...
           $cquery = "SELECT COUNT(*) AS dd FROM `#@__taglist` WHERE tid = '{$this->TagInfos['id']}' AND arcrank >-1 AND typeid in (78,77,79,80,81,82,83,84,85,86,87) ";
           ...
           $upquery = "UPDATE `#@__tagindex` SET total='{$row['dd']}',count=count+1,weekcc=weekcc+1,monthcc=monthcc+1 WHERE tag LIKE '{$this->Tag}' AND typeid in (78,77,79,80,81,82,83,84,85,86,87)";
            ...
            {
                $this->dsql->ExecuteNoneQuery("UPDATE `#@__tagindex` SET weekcc=0,weekup='{$ntime}' WHERE tag LIKE '{$this->Tag}' AND typeid in (78,77,79,80,81,82,83,84,85,86,87)");
            }

            //月统计
            if(ceil( ($ntime - $this->TagInfos['monthup'])/$oneday ) > 30)
            {
                $this->dsql->ExecuteNoneQuery("UPDATE `#@__tagindex` SET monthcc=0,monthup='{$ntime}' WHERE tag LIKE '{$this->Tag}' AND typeid in (78,77,79,80,81,82,83,84,85,86,87)");
            }
        }
    ...
    $imgwidth=120,$imgheight=90,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc')
    {
      ...
      //  $this->dsql->SetQuery("SELECT aid FROM `#@__taglist` WHERE tid = '{$this->TagInfos['id']}' AND arcrank>-1 LIMIT $limitstart,$getrow");
       $this->dsql->SetQuery("SELECT aid FROM `#@__taglist` WHERE tid = '{$this->TagInfos['id']}' AND arcrank>-1 AND typeid in (78,77,79,80,81,82,83,84,85,86,87) LIMIT $limitstart,$getrow");
        $this->dsql->Execute();
